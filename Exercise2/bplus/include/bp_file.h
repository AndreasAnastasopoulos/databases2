...
#define MAGIC_NUMBER 12345  // Unique identifier for a B+ tree file
#define TREE_ORDER 4        // Default order of the B+ tree

typedef struct {
    int file_desc;              // File descriptor from BF layer
    int root_block;             // Root block ID of the B+ tree
    int height;                 // Height of the B+ tree
    int order;                  // Order of the B+ tree (max children per node)
    ...
    int magic_number;           // Identifier to verify this is a B+ tree file
